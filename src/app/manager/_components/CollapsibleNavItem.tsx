...
import { type ManagerRoute } from "../_domain/routes";
...
export function CollapsibleNavItem({
  ...
  activeRoute: ManagerRoute & { displayMode: "link" | "text" };
  siblings: ManagerRoute[];
}) {
  ...
  const collapsibleContent = (
    ...
    >
      {siblings.map((s) => (
        ...
        >
          <Link href={s.path} onClick={handleLinkClick}>{s.name}</Link>
        </div>
      ))}
    </CollapsibleContent>
  );
      ...
      <div className="flex items-center justify-between space-x-2">
        <h4 className="text-sm font-semibold">
          {activeRoute.displayMode === "link" ? (
            <Link href={activeRoute.path} onClick={handleLinkClick}>{activeRoute.name}</Link>
          ) : (
            <>{activeRoute.name}</>
          )}
        </h4>
        {siblings.length > 0 && (
          <CollapsibleTrigger asChild>
            <Button variant="ghost" size="sm" className="px-0" >
              <ChevronsUpDown className="h-0 w-0" />
